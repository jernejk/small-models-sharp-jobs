...
public class IncidentDatasetTests
{
    ...
    [Fact]
    public void ValidateFilterRemovesGenericModelWordsBeforeGather()
    {
        var filter = IncidentDataset.ValidateFilter(new QueryFilter(null, null, "intersection crashes", 5));

        Assert.Equal("intersection", filter.Term);
        Assert.Equal(5, filter.MaxResults);
    }
}
