...
var prompt = Option("--prompt") ?? Option("--question") ?? "What patterns should an insurance and safety analyst review in the selected crash records?";
...
var records = IncidentDataset.Load(dataset);

try
{
    return command switch
    {
        "gather" => PrintGather(IncidentDataset.Gather(records, new IncidentQuery(from, to, term, max))),
...
int PrintGather(EvidencePack evidence)
{
...
async Task<int> TypedAsync()
{
    ...
    if (!check.IsValid)
        return ModelHint("typed", "malformed output: the reply did not parse into { action, target }");

    Console.WriteLine($"action: {check.Value!.Action}");
    Console.WriteLine($"target: {check.Value.Target}");
    Console.WriteLine($"elapsed: {check.Elapsed.TotalMilliseconds:F0} ms");
    return 0;
}
...
{
    var settings = ModelSettings.FromEnvironment();
    Console.WriteLine($"typed query | {settings.LaneLabel} | endpoint={settings.Endpoint} | model={settings.Model}");
    QueryCheck check;
    try { check = await new CrashPipeline(settings).InterpretQueryAsync(prompt); }
    catch (Exception ex) { return ModelError("query", ex); }
    Console.WriteLine("raw:\n" + check.Raw);
    if (!check.IsValid) return ModelHint("query", "malformed output: the reply did not parse into { from, to, term, maxResults }");
    Console.WriteLine("validated filter:\n" + WorkshopJson.Serialize(IncidentDataset.ValidateFilter(check.Value)));
    return 0;
}
...
async Task<int> RunAsync(string shape)
{
    ...
    if (from is not null || to is not null || term is not null)
    {
        evidence = IncidentDataset.Gather(records, new IncidentQuery(from, to, term, max));
        Console.WriteLine("filter source: explicit CLI options (deterministic)");
    }
    else
    {
        var querySettings = ModelSettings.FromEnvironment();
        QueryCheck interpreted;
        try { interpreted = await new CrashPipeline(querySettings).InterpretQueryAsync(prompt); }
        catch (Exception ex) { return ModelError("query", ex); }
        if (!interpreted.IsValid) return ModelHint("query", "malformed output: retry the typed query or use the deterministic debug override");
        var filter = IncidentDataset.ValidateFilter(interpreted.Value);
        evidence = IncidentDataset.Gather(records, filter);
        Console.WriteLine("filter source: tool-free typed QueryAgent, then C# validation");
        Console.WriteLine("validated filter: " + WorkshopJson.Serialize(filter));
    }
    ...
    var result = await new CrashPipeline(settings).RunAsync(prompt, evidence);
...
{
    var evidence = IncidentDataset.Gather(records, new IncidentQuery(from, to, term, max));
...
static int Usage()
{
    Console.WriteLine("""
        Usage: dotnet run --project src/Workshop.App -- <command> [options]
          ...
          query     CP-03 one tool-free typed query interpretation; C# validates its filter
        ...
        Debug overrides: --term text --from yyyy-MM-dd --to yyyy-MM-dd --max 1..20
        Other: --dataset file
        Compatibility: --question is an alias for --prompt.
