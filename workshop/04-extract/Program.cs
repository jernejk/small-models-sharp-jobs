...
Console.WriteLine($"extract | {settings.Lane} | model={settings.Model}");
...
try
{
    ...
    if (evidence.Count == 0)
    {
        Console.WriteLine("no evidence found: Extract is never called.");
    }
    else
    {
        CrashSelection? selection = null;

        // TODO: CP-04 ask the ExtractAgent for a CrashSelection over the evidence pack.
