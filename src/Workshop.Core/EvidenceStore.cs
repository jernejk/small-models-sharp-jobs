...
public sealed class EvidenceStore(string evidenceRoot)
{
    ...
    {
        // >>> TODO 1 | Only whitelisted evidence IDs may be read. Reject everything else. | STUB: throw new NotImplementedException("TODO 1: only whitelisted evidence IDs may be read");
        content = string.Empty;

        if (string.IsNullOrWhiteSpace(sourceId))
        {
            error = "empty evidence id";
            return false;
        }

        if (!IsWhitelisted(sourceId))
        {
            error = $"unknown evidence id '{sourceId}'";
            return false;
        }

        var resolved = Path.GetFullPath(Path.Combine(_root, sourceId));
        if (!string.Equals(resolved, Path.Combine(_root, sourceId), StringComparison.Ordinal))
        {
            error = $"resolved path escapes the evidence pack: '{sourceId}'";
            return false;
        }

        if (!File.Exists(resolved))
        {
            error = $"evidence file is missing on disk: '{sourceId}'";
            return false;
        }
