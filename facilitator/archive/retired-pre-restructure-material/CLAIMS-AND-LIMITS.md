...
| Numbered labs and facilitator reference compile | `scripts/validate-workshop-stages.sh`, `dotnet build facilitator/reference/solution/Workshop.slnx` |
...
| **LM Studio runs the full path**, on one machine, all gates green | `gates --repeat 5` via LM Studio 0.12.11 on an M4 Max, 24 Aug 2026 — see [LM-STUDIO-SWEEP.md](../../workshop/LM-STUDIO-SWEEP.md) |
...
- **LM Studio parity beyond one machine.** LM Studio now passes the same gates on an M4 Max with
  six models ([LM-STUDIO-SWEEP.md](../../workshop/LM-STUDIO-SWEEP.md)), so it is no longer untested — but that is
