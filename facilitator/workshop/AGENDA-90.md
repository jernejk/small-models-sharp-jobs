> Superseded 30 Aug: the delivery plan is [AGENDA-90.md → AGENDA-85.md](AGENDA-85.md). Kept for the segment detail.

# 85-minute agenda

The core is a local crash-review workflow: deterministic Victorian Gather -> typed Extract -> code gate -> typed Analyse -> code gate.
...
| 68-77 | Recovery / checkpoints | Pairing, saved Gather output, or a checkpoint solution. |
| 77-85 | Close and hidden extras | Model comparison, MCP and Harness only if time remains. |

Do not promise a loaded-model demo until `ready --prompt "Show up to 5 intersection crashes from 2012."` has completed on the presenter machine.
